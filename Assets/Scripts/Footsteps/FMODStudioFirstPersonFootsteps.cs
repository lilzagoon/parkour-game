...
public class FMODStudioFirstPersonFootsteps : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (PlayerTouchingGround && Input.GetButtonDown("Jump"))    // If the player is touching the ground AND the player presses the button to jump at the same time, we knoe that the player character is about to jump, therefore we perform our method to play a sound.
        {
