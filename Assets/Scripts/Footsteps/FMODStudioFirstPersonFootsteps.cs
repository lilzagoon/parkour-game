...
public class FMODStudioFirstPersonFootsteps : MonoBehaviour
{
    ...
    [SerializeField] FMODUnity.EventReference FootstepsEventPath;    // Use this in the Editor to select our Footsteps Event.
    [SerializeField] FMODUnity.EventReference JumpingEventPath;      // Use this in the Editor to select our Jumping Event.
    ...
    [SerializeField] private float RayDistance = 1.2f;                          // Select how far the raycast will travel down to when checking for a floor. This will then remain a constant and will not change.
                         // Set a tmie. If the time between each step the player takes is less than this value, the player will start to hear running footsteps. This will then remain a constant and will not change.
    ...
    void Update()
    {
        ...
        if (pm.grounded && Input.GetButtonDown("Jump"))    // If the player is touching the ground AND the player presses the button to jump at the same time, we knoe that the player character is about to jump, therefore we perform our method to play a sound.
        {
        ...
        if (!PreviosulyTouchingGround && pm.grounded)             // If the player wasn't touching the ground during the last frame, but is touching the ground during the current frame, then that means they must have just landed on the ground, therefore we perform out methods and play a sound.
        {
        ...
        if (DistanceTravelled >= StepDistance + runBoost + StepRandom)                  // If the distance the player has travlled is greater than or equal to the StepDistance plus the StepRandom, then we can perform our methods.
        {
    ...
    {
        if (pm.sprinting)                     // Remember that 'TimeTakenToStep' is a running timer. This method will only be performed as the player takes a step. So if this time is less than the value set to the 'StartRunningTime' variable, we can asume the player is moving fast enough to warrent a heavy running footstep to be played. So therefore...
        {
            runBoost = 2;
        ...
        }                                  // Finally, now that the player has taken the correct step, we reset our timer 'TimeTakenToStep' back to 0. Now th timer is tracking how much time has passed since the player took that last step.
    }
    ...
    {
        int layerMask = 1 << 3;
        if (Physics.Raycast(transform.position, Vector3.down, out hit, RayDistance, layerMask))                                 // A raycast is fired down, from the position that the player is curenntly standing at, traveling as far as we decide to set the 'RayDistance' variable to. Infomration about the object it comes into contact with will then be stored inside the 'hit' variable for us to access.
        {
    ...
    {
        if (Physics.Raycast(transform.position, Vector3.down, out hit, RayDistance))   // First, a raycast is fired from the location of the player, downwards towards the ground. It travels for as far as we set the 'RayDistance' variable to in UNity's Inspector and then stores information about whatever it comes into contact with inside the 'hit' variable. 
            ...
            {                                                                       // Else if however, there is no collider component...
    ...
    {
        if (pm.grounded)                                                                                    // First we check to see the player is touching the ground.
        {
    ...
    void WallFootsteps()
    {
        if (pm.wallrunning)
        {
            FMOD.Studio.EventInstance Footstep = FMODUnity.RuntimeManager.CreateInstance(FootstepsEventPath);        // If they are, we create an FMOD event instance. We use the event path inside the 'FootstepsEventPath' variable to find the event we want to play.
            FMODUnity.RuntimeManager.AttachInstanceToGameObject(Footstep, transform, GetComponent<Rigidbody>());     // Next that event instance is told to play at the location that our player is currently at.
            Footstep.setParameterByName(MaterialParameterName, 0);                                     // Before the event is played, we set the Material Parameter to match the value of the 'F_MaterialValue' variable.
            Footstep.setParameterByName(SpeedParameterName, F_PlayerRunning);                                        // We also set the Speed Parameter to match the value of the 'F_PlayerRunning' variable.
            Footstep.start();                                                                                        // We then play a footstep!.
            Footstep.release();
        }
